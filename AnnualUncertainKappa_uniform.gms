$ontext
Solve G-M model with uncertain kappa with uniform distribution
...
weights(n) = (kappa1-kappa0)/(card(n)-1)*2/3;
weights('1') = (kappa1-kappa0)/(card(n)-1)/3;
weights(n)$(ord(n)=card(n)) = (kappa1-kappa0)/(card(n)-1)/3;
loop(evenn$(ord(evenn)<=card(n)/2),
  weights(n)$(ord(n)=2*ord(evenn)) = (kappa1-kappa0)/(card(n)-1)*4/3;
);
...
file AnnualUncertainKappa_uniform_sol;
put AnnualUncertainKappa_uniform_sol;
AnnualUncertainKappa_uniform_sol.nw = 12;
AnnualUncertainKappa_uniform_sol.nr = 2;
AnnualUncertainKappa_uniform_sol.nz = 1e-15;
loop(t,
    put t.tl:4:0;
    put S.l(t):14:6;
    put A.l(t):14:6;
